...
"""
The LTPTrainer class, to easily train a 🤗 Transformers from scratch or finetune it on a new task.
"""
...
class LTPTrainer:
    """
    LTPTrainer is a simple but feature-complete training and eval loop for PyTorch, optimized for 🤗 Transformers.
    ...
    Args:
        model ([`PreTrainedModel`] or `torch.nn.Module`, *optional*):
            ...
            [`LTPTrainer`] is optimized to work with the [`PreTrainedModel`] provided by the library. You can still use
        ...
        train_dataset (`torch.utils.data.Dataset` or `torch.utils.data.IterableDataset`, *optional*):
            ...
            `torch.Generator` for the randomization that must be identical on all processes (and the LTPTrainer will
        ...
        model_init (`Callable[[], PreTrainedModel]`, *optional*):
            A function that instantiates the model to be used. If provided, each call to [`~LTPTrainer.train`] will start
        ...
        callbacks (List of [`TrainerCallback`], *optional*):
            ...
            If you want to remove one of the default callbacks used, use the [`LTPTrainer.remove_callback`] method.
    ...
    def __init__(
        ...
        if model is None:
            if model_init is not None:
                ...
                raise RuntimeError("`LTPTrainer` requires either a `model` or `model_init` argument")
        else:
            if model_init is not None:
                warnings.warn(
                    "`LTPTrainer` requires either a `model` or `model_init` argument, but not both. `model_init` will"
        ...
        if model_init is not None and (self.optimizer is not None or self.lr_scheduler is not None):
            raise RuntimeError(
                ...
                "You should subclass `LTPTrainer` and override the `create_optimizer_and_scheduler` method."
            )
    ...
    def get_train_dataloader(self) -> DataLoader:
        ...
        if self.train_dataset is None:
            raise ValueError("LTPTrainer: training requires a train_dataset.")
    ...
    def get_eval_dataloader(self, eval_dataset: Optional[Dataset] = None) -> DataLoader:
        ...
        if eval_dataset is None and self.eval_dataset is None:
            raise ValueError("LTPTrainer: evaluation requires an eval_dataset.")
    ...
    def create_optimizer_and_scheduler(self, num_training_steps: int):
        ...
        LTPTrainer's init through `optimizers`, or subclass and override this method (or `create_optimizer` and/or
    ...
    def create_optimizer(self):
        ...
        We provide a reasonable default that works well. If you want to use something else, you can pass a tuple in the
        LTPTrainer's init through `optimizers`, or subclass and override this method in a subclass.
        """
        ...
        if self.optimizer is None:
            ...
            optimizer_grouped_parameters = [
                {
                    "params": [p for n, p in opt_model.named_parameters() if n in decay_parameters and 'threshold' not in n],
                    "weight_decay": self.args.weight_decay,
                },
                {
                    "params": [p for n, p in opt_model.named_parameters() if n not in decay_parameters and 'threshold' not in n],
                    "weight_decay": 0.0,
                },
            ]
            ...
            optimizer_cls, optimizer_kwargs, optimizer_threshold_kwargs = LTPTrainer.get_optimizer_cls_and_kwargs(self.args)
    ...
    def get_optimizer_cls_and_kwargs(args: TrainingArguments) -> Tuple[Any, Any]:
        ...
        if args.optim == OptimizerNames.ADAFACTOR:
            ...
            try:
                ...
                raise ValueError("LTPTrainer failed to import syncfree AdamW from torch_xla.")
            ...
            try:
                ...
                raise ValueError("LTPTrainer tried to instantiate apex FusedAdam but apex is not installed!")
            ...
            try:
                ...
                raise ValueError("LTPTrainer tried to instantiate bnb Adam8bit but bnb is not installed!")
        elif args.optim == OptimizerNames.SGD:
            optimizer_cls = torch.optim.SGD
        elif args.optim == OptimizerNames.ADAGRAD:
            optimizer_cls = torch.optim.Adagrad
        else:
            raise ValueError(f"LTPTrainer cannot instantiate unsupported optimizer: {args.optim}")

        if args.lr_threshold is not None:
            optimizer_threshold_kwargs["lr"] = args.lr_threshold
        else:
            optimizer_threshold_kwargs = optimizer_kwargs

        return optimizer_cls, optimizer_kwargs, optimizer_threshold_kwargs
    ...
    def create_scheduler(self, num_training_steps: int, optimizer: torch.optim.Optimizer = None):
        ...
        if self.lr_scheduler is None:
            ...
            self.lr_scheduler_threshold = get_scheduler(
                ...
                num_warmup_steps=self.args.get_warmup_steps(threshold_training_steps),
        ...
        **kwargs,
        ...
        Args:
            resume_from_checkpoint (`str` or `bool`, *optional*):
                If a `str`, local path to a saved checkpoint as saved by a previous instance of [`LTPTrainer`]. If a
                `bool` and equals `True`, load the last checkpoint in *args.output_dir* as saved by a previous instance
                of [`LTPTrainer`]. If present, training will resume from the model/optimizer/scheduler states loaded here.
    ...
    def hyperparameter_search(
        ...
        **kwargs,
        ...
        To use this method, you need to have provided a `model_init` when initializing your [`LTPTrainer`]: we need to
        reinitialize the model at each new run. This is incompatible with the `optimizers` argument, so you need to
        subclass [`LTPTrainer`] and override the method [`~LTPTrainer.create_optimizer_and_scheduler`] for custom
        optimizer/scheduler.
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        """
        How the loss is computed by LTPTrainer. By default, all models return the loss in the first element.
    ...
    def _save_tpu(self, output_dir: Optional[str] = None):
        ...
        if not isinstance(self.model, PreTrainedModel):
            if isinstance(unwrap_model(self.model), PreTrainedModel):
                ...
                logger.info("LTPTrainer.model is not a `PreTrainedModel`, only saving its state dict.")
                state_dict = self.model.state_dict()
    ...
    def evaluation_loop(
        ...
        """
        Prediction/evaluation loop, shared by `LTPTrainer.evaluate()` and `LTPTrainer.predict()`.
    ...
    def push_to_hub(self, commit_message: Optional[str] = "End of training", blocking: bool = True, **kwargs) -> str:
        ...
        Parameters:
            ...
            kwargs:
                Additional keyword arguments passed along to [`~LTPTrainer.create_model_card`].
