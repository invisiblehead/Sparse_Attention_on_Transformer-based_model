...
def main():
    ...
    if args.task_name is not None and args.task_name.lower()!='code':
        # Downloading and loading a dataset from the hub.
        raw_datasets = load_dataset("glue", args.task_name)
    else:
        # TODO: from here, check the logic and replace the dataset
        ...
        # TODO; sanity check the logic here when you load your own dataset
    ...
    if args.task_name is not None and args.task_name.lower()!='code':
        ...
        non_label_column_names = [name for name in raw_datasets["train"].column_names if name == "functionSource"]
    ...
    for step, batch in enumerate(eval_dataloader):
        ...
        for i in range(batch_size):
            ...
            tokens = [id2token(x).replace('Ġ', '') for x in ids]
            for rel_idx in rest_indices[i]:
                tokens[rel_idx] = f"{bcolors.WARNING}{tokens[rel_idx]}{bcolors.ENDC}"
            print(" ".join(tokens))
