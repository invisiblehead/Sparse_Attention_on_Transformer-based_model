...
def arg_parse():
    ...
    parser.add_argument('--task', type=str, help='finetuning task',
                        choices=['RTE', 'SST2', 'MNLI', 'QNLI', 'QQP', 'MRPC', 'STSB', 'CODE'])
    parser.add_argument('--train_file', type=str, default=None)
    parser.add_argument('--validation_file', type=str, default=None)
    parser.add_argument('--test_file', type=str, default=None)
    parser.add_argument('--eval', action='store_true')
    # parser.add_argument('--skip_train', action='store_false', default=False)
...
if 'ltp' in args.arch:
    run_file = './run_glue_ltp_code_metrics.py'
else:
    run_file = './run_glue_code_metrics.py'
...
if not args.eval:
    ...
    if args.task.lower() == 'code':
        if args.train_file is not None:
            subprocess_args += ['--train_file', args.train_file]
        if args.validation_file is not None:
            subprocess_args += ['--validation_file', args.validation_file]
        if args.test_file is not None:
            subprocess_args += ['--test_file', args.test_file]
    if not args.no_load:
...
# Eval-only mode
    ...
    # ####### we still need the train/validation set for the cmd task
    if args.task.lower() == 'code':
        if args.train_file is not None:
            subprocess_args += ['--train_file', args.train_file]
        if args.validation_file is not None:
